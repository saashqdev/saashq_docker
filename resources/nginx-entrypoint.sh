...
if [[ -z "$SAASHQ_SITE_NAME_HEADER" ]]; then
  # shellcheck disable=SC2016
  echo 'SAASHQ_SITE_NAME_HEADER defaulting to $host'
  # shellcheck disable=SC2016
  export SAASHQ_SITE_NAME_HEADER='$host'
fi
...
envsubst '${BACKEND}
  ...
  </templates/nginx/saashq.conf.template >/etc/nginx/conf.d/saashq.conf
