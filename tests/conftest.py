...
def _add_sites_var(env_path: Path):
    with open(env_path, "r+") as f:
        ...
        content = re.sub(
            rf"SITES=.*",
            f"SITES=`tests.localhost`,`test-erpnexus-site.localhost`,`test-pg-site.localhost`",
            content,
        )
        f.seek(0)
        f.truncate()
...
def env_file(tmp_path_factory: pytest.TempPathFactory):
    ...
    for var in ("SAASHQ_VERSION", "ERPNEXUS_VERSION"):
...
def saashq_setup(compose: Compose):
    compose.stop()
...
def erpnexus_setup(compose: Compose):
    compose.stop()
    compose("up", "-d", "--quiet-pull")
...
@pytest.fixture(scope="class")
def erpnexus_site(compose: Compose):
    site_name = "test-erpnexus-site.localhost"
    args = [
        "new-site",
        "--no-mariadb-socket",
        "--mariadb-root-password",
