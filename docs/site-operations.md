...
```sh
docker-compose exec backend wrench new-site --no-mariadb-socket --mariadb-root-password <db-password> --admin-password <admin-password> <site-name>
```

If you need to install some app, specify `--install-app`. To see all options, just run `wrench new-site --help`.
...
```sh
docker-compose exec backend wrench set-config -g root_login <root-login>
docker-compose exec backend wrench set-config -g root_password <root-password>
```

Also command is slightly different:

```sh
docker-compose exec backend wrench new-site --no-mariadb-socket --db-type postgres --admin-password <admin-password> <site-name>
```
...
For reference of commands like `backup`, `drop-site` or `migrate` check [official guide](https://saashqframework.com/docs/v13/user/en/wrench/saashq-commands) or run:

```sh
docker-compose exec backend wrench --help
```
...
```sh
docker-compose exec backend wrench --site <site-name> migrate
```
