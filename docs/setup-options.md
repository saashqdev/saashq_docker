...
### Setup Saashq without proxy and external MariaDB and Redis
...
### Setup ERPNexus with proxy and external MariaDB and Redis
...
### Setup Saashq using containerized MariaDB and Redis with Letsencrypt certificates.
...
### Setup ERPNexus using containerized MariaDB and Redis with Letsencrypt certificates.
...
Switch to the root of the `saashq_docker` directory before running the following commands:

```sh
# Update environment variables ERPNEXUS_VERSION and SAASHQ_VERSION
nano .env
